...
from collections.abc import Iterable as c_abc_Iterable, Sequence as fs, Set as fset, Mapping as fm, MutableSequence as ms, MutableSet as mset, MutableMapping as mm
...
def Slice_simplifier(obj, key, value=None, /, *, mode: str, base_action: Callable[[], T]) ->T:
    if not obj._normalize_slice or not isinstance(key, slice): return base_action()
    
    indices=range(*key.indices(len(obj) ) )
    
    if mode[1:]=='get': return (tuple if mode[0]=='i' else list)(obj[i] for i in indices)
    elif mode=='set': 
        if not isinstance(value, c_abc_Iterable): raise TypeError("must assign iterable to extended slice")
        elif indices.step==1:
            del obj[key]
            for i, e in enumerate(value): obj[indices.start+i]=e
        else:
            if len(indices)!=len(value): raise ValueError(f"attempt to assign sequence of size {len(value)} to extended slice of {len(indices)}")
            for i, v in zip(indices, value): obj[i]=v
    else:
        for i in indices: del obj[i]
...
class ManipulatorProtocol:
    ...
    def create(self, obj) ->None: print("op=create")
        
    def iterate(self, obj, base_action: Callable[[], T]) ->T: print("op=iterate"); return base_action()
        
    def get(self, obj, base_action: Callable[[] ,T], key) ->T: print(f"op=get, {key=}"); return base_action()
        
    def set(self, obj, base_action: Callable[[], None], value, key=None): print(f"op=set, {key=}, {value=}"); base_action()
        
    def insert(self, obj, base_action: Callable[[], None], value, key=None) ->None: print(f"op=insert, {key=}, {value=}"); base_action() 
        
    def delete(self, obj, base_action: Callable[[], None], key): print(f"op=delete, {key=}"); base_action()
...
class BaseContainerType[T]:
    ...
    def __init__(self, manipulator) ->None: self._manipulator=manipulator; self._notify("create")
    
    #base container ops
    def __eq__(self, other) ->bool: return self._values == other if not hasattr(other, '_values') else other._values
    ...
    def _notify(self, action_taken: str, /, *data, default: Optional[Callable[[], Any] ]=None) ->Any:
        ...
        f=getattr(self._manipulator, action_taken)
        return f(self, default, *data) if data else (f(self, default) if callable(default) else f(self) )
...
class ManipulatorTuple[T](BaseContainerType[T], fs):
    ...
    def __init__(self, manipulator, it: Iterable[T]=(), /, *, normalize_slice: bool =False) ->None: self._values, self._normalize_slice=tuple(it), normalize_slice; super().__init__(manipulator)
    
    def __getitem__(self, key) ->Any: return Slice_simplifier(
                                                 self, key, mode='iget',
                                                 base_action=lambda: self._notify(
                                                                         "get", key, 
                                                                         default=lambda: self._values[key]
                                                                                 )
                                                             )
...
class ManipulatorFrozenDict[T, U](BaseContainerType[T], fm):
    ...
    def __init__(self, manipulator, it: Iterable[tuple[T, U] ]=(), /, **kwargs) ->None: self._values=mpt(dict(it)|kwargs); super().__init__(manipulator)
...
class ManipulatorFrozenSet[T](BaseContainerType[T], fset):
    ...
    def __init__(self, manipulator, it: Iterable[Hashable]=() ) ->None: self._values=frozenset(it); super().__init__(manipulator)
...
class ManipulatorList[T](BaseContainerType[T], ms):
    ...
    def __init__(self, manipulator, it: Iterable[T]=(), /, *, normalize_slice: bool =False) ->None: self._values, self._normalize_slice=list(it), normalize_slice; super().__init__(manipulator)
    ...
    def __setitem__(self, key, value) ->None: Slice_simplifier(
                                                  self, key, value, mode="set", 
                                                  base_action=lambda: self._notify(
                                                                          "set", value, key,
                                                                          default=lambda: self._values.__setitem__(key, value)
                                                                                  ) 
                                                              )
    
    def __delitem__(self, key) ->None: Slice_simplifier(
                                           self, key, mode="delete",
                                           base_action=lambda: self._notify(
                                                                   "delete", key,
                                                                   default=lambda: self._values.__delitem__(key)
                                                                           ) 
                                                       )
...
class ManipulatorSet[T](BaseContainerType[T], mset):
    ...
    def __init__(self, manipulator, it: Iterable[Hashable]=() ) ->None: self._values=set(it); super().__init__(manipulator)
    ...
    def update(self, it: Iterable[Hashable]) ->None:
        for i in it: self.add(i)
    
    def discard(self, other: Hashable) ->None: 
...
class ManipulatorDict[T, U](BaseContainerType[T], mm):
    ...
    def __init__(self, manipulator, it: Iterable[tuple[T, U] ]=(), /, **kwargs) ->None: self._values=dict(it)|kwargs; super().__init__(manipulator)
...
class LifetimeM:
    ...
    def create(self, obj) ->None: self.items={i:self.lifespan for i in obj._values}#; open_collector_slot(obj)
    
    def iterate(self, obj, base_action: Callable[[], T]) ->T:
        it=iter(obj._values.copy() )
        for i in obj._values:
            self.items[i]-=1
            if self.items[i] is Dead: getattr(obj, "pop" if isinstance(obj, ManipulatorDict) else "discard")(i)
        return it
    ...
    def set(self, obj, base_action: Callable[[], None], value: Any, key: object|Hashable=missing) ->None: base_action(); self.items.setdefault(value if key is missing else key, self.lifespan)
    
    def delete(self, obj, base_action: Callable[[], None], value: Hashable) ->None: base_action(); del self.items[value]
...
class SizedM:
    ...
    def create(self, obj) ->None:
        ...
        elif any(i < 0 or type(i)!=int for i in self.capacity): raise ValueError("min_size and max_size must be non-negetive integers")
        self.delete(obj, lambda: None, None); self.set(obj, lambda: None, None)
            
    def set(self, obj, base_action: Callable[[], None], value, key: Optional[Hashable| int]=None) ->None:
        base_action()
        if len(obj) > self.max_size: raise OverflowError(f"maximum capacity violeted, limit:- {self.max_size}")
    
    def delete(self, obj, base_action: Callable[[], None], value) ->None:
        base_action()
...
class TypedM:
    ...
    def set(self, obj, base_action, value, key: Optional[Hashable| int]=None) ->None:
...
class GroupM:
    ...
    def delete(self, obj, base_action, key) ->None:
        base_action()
...
class GroupType:
    ...
    def _check_grp_not_exists(self, name) ->None:
        if name in self._get_groups: raise ValueError(f"cannot override existing group '{name}': use '.change()' to override a group")
    
    def _check_grp_exists(self, name) ->None:
        if name not in self._get_groups: raise ValueError(f"invalid group name: '{name}' notfound")
    @property
    def _get_groups(self) ->dict[str, Iterable[Any] ]: return self._manipulator.groups
    
    def discard_group(self, target) ->None:
    ...
    def remove_group(self, target: str) ->None: self._error_for_nonexisting_grp(target); del self._get_groups[target]
    
    def clear_groups(self) ->None: self._get_groups.clear()
...
class TypedType:
    ...
    @property
    def allowed_types(self) ->tuple[type]: return self._manipulator.allowed_types


class RadioActiveType:
    def __init__(self, *args, **kwargs): super().__init__(RadioActiveM(), *args, **kwargs)
    
    def _del(self, target) ->None: del self[target]
...
if __name__=="__main__":
    m=ManipulatorList(ManipulatorProtocol(), (1,2,3,4,5,6,7,8) , normalize_slice=True)
    m[1:4]=(7,89)
    print(m)
